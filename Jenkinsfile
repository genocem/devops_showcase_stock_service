pipeline {
    ...
    environment {
        ...
        GIT_REPO_URL = 'https://github.com/genocem/devops_showcase_stock_service.git'
        GIT_BRANCH = 'main'
    ...
    stages {
        ...
        stage('image Scan') {
            steps {
                sh '''
                    trivy client --remote http://localhost:4954 image --severity HIGH,CRITICAL --report summary --output ${TRIVY_REPORT_IMAGE} --no-progress ${DOCKER_IMAGE} --format table
                '''
            }
            }
